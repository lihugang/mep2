    ...
    return new Promise<string>((resolve, reject) => {
        ...
        Promise.all(awaitPromises).then(async () => {
            ...
            if (isRenderWaterMark) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                ctx.fillText(window.i18n.template('generated_by', {
                    app: (document.querySelector('.platform')! as HTMLElement || { innerText: '' }).innerText
                }), canvas.width * 0.8, canvas.height * 0.98);
                const img = new Image();
